		...
		function handleTextureSelect(e) {
			// Procura o botão correto (caso o clique pegue no span ou emoji interno)
			...
			// Atualizar seleção visual (remove active dos outros e adiciona no atual)
			document.querySelectorAll(`[data-type="${type}"]`).forEach(btn => {
				btn.classList.remove('active');
			});
			targetBtn.classList.add('active');
			
			// Atualizar textura atual na variável global
			currentTextures[type] = textureId;
			
			// Atualizar inputs de cores
			const preset = texturePresets.find(t => t.id === textureId);
			
			// NOTA: O código abaixo muda as cores APENAS se for a Primária.
			// Se quiser que a Secundária também mude as cores, remova o "&& type === 'primary'"
			if (preset) {
			...
			// --- CORREÇÃO IMPORTANTE ---
			// Força a regeneração da textura imediatamente para ver o resultado
			generateTexture();
		}
		...
		function setRenderMode(mode) {
			...
			// Gera a textura imediatamente para ver o efeito
			generateTexture();
		}
        ...
        function generateTexture() {
            ...
            const data = imageData.data;
                ...
                for (let x = 0; x < size; x++) {
                    ...
                    // Lógica baseada no MODO DE RENDERIZAÇÃO
                    let value;

                    if (currentRenderMode === 'solid') {
                        // Modo Sólido: Valor constante (pega a cor predominante do gradiente)
                        value = 0.5; 
                    
                    } else if (currentRenderMode === 'gradient') {
                        // Modo Gradiente: Usa apenas coordenadas X e Y, ignora ruído
                        // Cria um gradiente diagonal suave
                        value = (x / size) * 0.5 + (y / size) * 0.5;
                        
                        // Opcional: Adiciona leve distorção se o slider estiver alto
                        if(params.distortion > 0) {
                            value += noise(x * 0.005, y * 0.005, params.seed) * params.distortion * 0.2;
                        }

                    } else if (currentRenderMode === 'pixel') {
                        // Modo Pixel Art: "Quantiza" as coordenadas
                        const pixelSize = 16; // Tamanho do pixelzão
                        const px = Math.floor(x / pixelSize) * pixelSize;
                        const py = Math.floor(y / pixelSize) * pixelSize;
                        
                        // Usa a mesma lógica da textura normal, mas com coordenadas travadas
                    ...
                    // Aplicar efeitos especiais (se não for sólido)
                    if (currentRenderMode !== 'solid') {
                         activeEffects.forEach(effect => {
                            // Pequeno ajuste para o modo pixel não quebrar o efeito
