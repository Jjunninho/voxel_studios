...
function generateTexture() {
    ...
    for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
            ...
            } else if (currentRenderMode === 'normal') {
                // ========================================
                // NOVO: Normal Map Mode
                // ========================================
                const nx = x / size;
                const ny = y / size;
                
                // Calcula a altura baseada na textura
                let height = generateTextureFunction(nx, ny, params, currentTextures.primary);
                
                // Calcula as derivadas (gradientes) para X e Y
                const delta = 1 / size;
                const heightRight = generateTextureFunction(nx + delta, ny, params, currentTextures.primary);
                const heightUp = generateTextureFunction(nx, ny + delta, params, currentTextures.primary);
                
                const dx = (heightRight - height) * 10; // Multiplica para intensificar
                const dy = (heightUp - height) * 10;
                
                // Converte para RGB (Normal Map)
                // R = X, G = Y, B = Z (sempre positivo)
                const normalX = (dx * 0.5 + 0.5) * 255;
                const normalY = (dy * 0.5 + 0.5) * 255;
                const normalZ = 200; // Z é sempre positivo no normal map
                
                data[i] = normalX;
                data[i + 1] = normalY;
                data[i + 2] = normalZ;
                data[i + 3] = 255;
                continue; // Pula o resto do loop para este pixel
            } else {
                // Modo TEXTURA
            ...
            // Aplicar efeitos (não aplica em normal map)
            if (currentRenderMode !== 'solid' && currentRenderMode !== 'normal') {
    ...
    // APENAS verifica saturação em modo TEXTURE
    if (currentRenderMode === 'texture') {
        ...
        // Modos especiais não verificam saturação
        infoContainer.innerHTML = `Visualização da Textura - Modo: ${currentRenderMode.toUpperCase()}`;
        visualVariance = 999; // Retorna valor alto para não quebrar o Guardião
    }
