...
// Botão de seed aleatória (Apenas muda o número, não inicia o Guardião)
...
// --- CORE: Gerar Textura (Modo Síncrono para Preview Rápido nos Sliders) ---
...
// --- SISTEMA DE RANDOMIZAÇÃO (ASSÍNCRONO) ---

// Randomiza valores da UI
...
function requestTextureFromWorker(attempt) {
    ...
    params.textureConfig.seed = Math.floor(Math.random() * 100000); // Tenta seed nova
...
worker.onmessage = (e) => {
    ...
    if (variance > VARIANCE_THRESHOLD || attemptCount > 20 || currentRenderMode === 'solid') {
        ...
        // Atualiza JSON final para refletir a seed vencedora
        // Chamamos generateTexture sem alterar a imagem, apenas para atualizar o objeto recipe
        // (Isso é uma pequena ineficiência aceitável para manter o JSON sincronizado)
        // generateTexture(); 
