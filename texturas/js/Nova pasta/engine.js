...
// Função auxiliar para pegar altura com Tiling (loop infinito)
function getHeight(x, y, w, h, map, isTiling) {
    let sx = x;
    let sy = y;

    if (isTiling) {
        // Operador Modulo correto para JavaScript (trata negativos)
        sx = ((x % w) + w) % w;
        sy = ((y % h) + h) % h;
    } else {
        // Clamp nas bordas se não for tiling
        sx = Math.max(0, Math.min(w - 1, x));
        sy = Math.max(0, Math.min(h - 1, y));
    }
    
    return map[sy * w + sx];
}

export function calculateNormal(x, y, w, h, heightMap, isTiling) {
    const strength = 2.0; // Intensidade do relevo

    // Sobel Kernel
    // T = Top, B = Bottom, L = Left, R = Right, M = Mid
    // TL = Top-Left, etc.
    
    const tl = getHeight(x-1, y-1, w, h, heightMap, isTiling);
    const t  = getHeight(x  , y-1, w, h, heightMap, isTiling);
    const tr = getHeight(x+1, y-1, w, h, heightMap, isTiling);
    const l  = getHeight(x-1, y  , w, h, heightMap, isTiling);
    const r  = getHeight(x+1, y  , w, h, heightMap, isTiling);
    const bl = getHeight(x-1, y+1, w, h, heightMap, isTiling);
    const b  = getHeight(x  , y+1, w, h, heightMap, isTiling);
    const br = getHeight(x+1, y+1, w, h, heightMap, isTiling);
    ...
    // Normalização do vetor (Matemática pura)
...
// ... manter generateTextureFunction existente ...