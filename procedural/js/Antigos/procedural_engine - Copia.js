...
async generate(shapeId, params = {}) {
        // Busca no registro global
        const shape = (typeof ShapeRegistry !== 'undefined' ? ShapeRegistry : window.ShapeRegistry)?.[shapeId];
        
        if (!shape) {
            console.error('❌ Shape não encontrada no ShapeRegistry:', shapeId);
        ...
        try {
            ...
            // 🔥 INTERCEPTOR MELHORADO: Captura escala e rotação também
            window.addBlockAt = (x, y, z, color, type, scale, rotation) => {
                capturedBlocks.push({ x, y, z, color, type, scale, rotation });
            };
    ...
    // Adicionar bloco visual à cena
    addBlockToScene(blockData) {
        const { x, y, z, color, type } = blockData;
        
        const geometry = new THREE.BoxGeometry(1, 1, 1);
        
        // Converter cor (pode ser hex string ou number)
        let colorValue = color || 0x00ff00;
        if (typeof color === 'string') {
            colorValue = parseInt(color.replace('#', '0x'));
        }
