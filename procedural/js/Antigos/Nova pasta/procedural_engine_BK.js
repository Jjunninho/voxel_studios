...
// Em ProceduralEngine
    ...
    addBlockToScene(blockData) {
		...
		// 🔥 IMPORTANTE: Salvar o tipo nos dados do Mesh para recuperar na exportação
	...
	// 2. ATUALIZAR exportJSON PARA O FORMATO COMPATÍVEL
    exportJSON() {
        const data = {
            ...
            blocks: this.generatedBlocks.map(mesh => ({
                ...
                type: mesh.userData.type || 'cube', // Recupera o tipo salvo
                scale: {
                    x: mesh.scale.x,
                    y: mesh.scale.y,
                    z: mesh.scale.z
        ...
        this.showToast('JSON exportado (Formato Voxel Genesis)!', 'success');
    },
