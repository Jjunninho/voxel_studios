...
renderInventorySlots() {
        ...
        // Pega lista definida manualmente no categoryMap
        const categories = ProceduralEngine.categoryMap;
        
        // Para cada categoria definida manualmente
        Object.entries(categories).forEach(([catName, shapeIds]) => {
            
            shapeIds.forEach(shapeId => {
                // Tenta pegar a configuração
                const config = ProceduralEngine.getGeneratorConfig(shapeId);
                
                // 🕵️ DEBUG 1: Se o botão não aparece, avisa o motivo
                if (!config) {
                    console.error(`🚨 ERRO DE CONFIG: O item '${shapeId}' está no menu (categoryMap) mas NÃO EXISTE no ShapeRegistry! Verifique se o arquivo .js foi carregado.`);
                    
                    // Opcional: Cria um "botão fantasma" vermelho pra você saber que faltou
                    const errorSlot = document.createElement('div');
                    errorSlot.className = 'inv-slot error-slot';
                    errorSlot.style.border = '1px solid red';
                    errorSlot.innerHTML = `<div class="icon">❌</div><div class="name">${shapeId}</div>`;
                    errorSlot.title = "Erro: Shape não encontrada";
                    grid.appendChild(errorSlot);
                    return; 
                }
                ...
                slot.dataset.subcategory = config.subcategory || 'geral'; // Suporte a subcategoria
                ...
                // --- 🕵️ DEBUG 2: O QUE ACONTECE AO CLICAR ---
                slot.addEventListener('click', () => {
                    console.group(`🔍 DIAGNÓSTICO: ${config.name} [${shapeId}]`);
                    
                    console.log(`📂 Categoria no Menu: "${catName}"`);
                    console.log(`📑 Subcategoria: "${config.subcategory || 'Nenhuma'}"`);
                    
                    const shapeObj = window.ShapeRegistry[shapeId];
                    console.log(`📦 Objeto no Registry:`, shapeObj);
                    
                    if (shapeObj) {
                        console.log(`✅ Função 'generate' existe?`, typeof shapeObj.generate === 'function' ? "SIM" : "NÃO ❌");
                        console.log(`🎛️ Parâmetros esperados:`, shapeObj.params);
                        
                        if (typeof shapeObj.generate !== 'function') {
                            console.error(`❌ CRÍTICO: A forma existe, mas não tem a função .generate()!`);
                        }
                    } else {
                        console.error(`❌ CRÍTICO: Objeto undefined no Registry (Isso não deveria acontecer se o botão apareceu)`);
                    }
        ...
        // (O resto dos listeners de tabs e search continua igual aqui embaixo...)
        // ...
        this.setupInventoryFilters(); // Se você tiver separado essa função
    },
